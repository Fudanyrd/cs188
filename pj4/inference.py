...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        # join query factors by variable
        # for joinVariable in queryVariables: # bayesNet.variablesSet():
        #    currentFactorsList, joinedFactor = joinFactorsByVariable(currentFactorsList, joinVariable)
        #    currentFactorsList.append(joinedFactor)
        # join eliminate variables and eliminate them one by one.
        for eliminationVariable in eliminationVariables:
...
class ExactInference(InferenceModule):
    ...
    def elapseTime(self, gameState: busters.GameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Note: getPositionDistribution is fairly expensive, reduce 
        # number of calls as much as you can.
        # newPosDist = self.getPositionDistribution(gameState, ghost)
        # return
        old_beliefs = {}
        for pos in self.allPositions:
            old_beliefs[pos] = self.beliefs[pos]
            self.beliefs[pos] = 0.0
        for pos in self.allPositions:
            newPosDist = self.getPositionDistribution(gameState, pos)
            for key in list(newPosDist.keys()):
                self.beliefs[key] += old_beliefs[pos] * newPosDist[key]
        "*** END YOUR CODE HERE ***"
...
class ParticleFilter(InferenceModule):
    ...
    def elapseTime(self, gameState):
        ...
        for pos in self.legalPositions:
            newPosDist = self.getPositionDistribution(gameState, pos)
            for key in list(newPosDist.keys()):
                dist[key] += newPosDist[key] * self.particles.count(pos) / len(self.particles)
        dist.normalize()
        for pos in self.legalPositions:
            for i in range(int(self.numParticles * dist[pos])):
...
class JointParticleFilter(ParticleFilter):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        for t in tuples:
            for i in range(self.numParticles // len(tuples)):
