...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState: busters.GameState):
        ...
        for i in range(len(livingGhosts)):
            if livingGhosts[i]:
                probs = livingGhostPositionDistributions[idx]
                # find the max key with the max probs.
                maxProb = 0.0
                key = None
                ...
                mostProb.append(None)
        idx = 0
        leastDist = float('inf')
        for i in range(len(mostProb)):
            if mostProb[i] is None:
                continue
            dist = self.distancer.getDistance(gameState.getPacmanPosition(), mostProb[i])
            if leastDist > dist:
                leastDist = dist
                idx = i
        # your destination is mostProb[idx].
        optimal = None
        leastDist = float('inf')
        for action in gameState.getLegalActions():
            dist = self.distancer.getDistance(mostProb[idx], Actions.getSuccessor(gameState.getPacmanPosition(), action))
            if dist < leastDist:
                optimal = action
                leastDist = dist
        return optimal
