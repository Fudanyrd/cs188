...
class FullyConnected(Layer):
    ...
    def _init_parameters(self, X_shape: Tuple[int, int]) -> None:
        ...
        S = np.zeros((X_shape[0], self.n_out))  # affine combination.
        X = np.zeros(X_shape)
        grad_W = np.zeros(W.shape)
        grad_b = np.zeros(b.shape)

        self.parameters = OrderedDict({"W": W, "b": b})
        self.cache = OrderedDict({"S": S, "X": X})  # cache for backprop
        self.gradients = OrderedDict({"W": grad_W, "b": grad_b})
    ...
    def backward(self, dLdY: np.ndarray) -> np.ndarray:
        ...
        # unpack the cache
        S = self.cache['S'] 
        dLdS = self.activation.backward(S, dLdY)
        # compute the gradients of the loss w.r.t. all parameters as well as the
        # input of the layer

        dX = dLdS @ self.parameters["W"].T

        # store the gradients in `self.gradients`
        # the gradient for self.parameters["W"] should be stored in
        # self.gradients["W"], etc.
        self.gradients["W"] = self.cache["X"].T @ dLdS # dLdS @ self.parameters["W"]
        ones = np.ones((1, dLdS.shape[0]))
        self.gradients["b"] = ones @ dLdS
